...
```
```{r pkg-import-1, include=F, echo=F}
# library("tokenizers")
# library("pdftools")
# filter <- dplyr::filter
library("tidyverse")
...
```
```{r viz-vars-1, include=F, eval=F, echo=F}
...
```
```{r download_report-1, include=T, eval=F, echo=T}
years <- 2016L:2018L
...
```
```{r paths-1, include=F, eval=T, echo=F}
paths <-
  list.files(
    path = "data-raw",
    pattern = "pdf$",
...
```
```{r paths_INFO-1, include=T, eval=T, echo=T}
paths_info <-
  paths %>%
  tibble(path = .) %>%
  mutate(
...
```
```{r pages_nest-1, include=T, eval=T, echo=T}
pages_nest <-
  paths_info %>%
  mutate(
...
```
```{r pages-1, include=T, eval=T, echo=T}
pages <-
  pages_nest %>%
  unnest(page) %>%
  group_by(year) %>%
...
```
```{r pages_n-1, include=T, eval=T, echo=T}
pages_n <-
  pages_nest %>%
...
```
```{r toc_pages-1, include=T, eval=T, echo=T}
...
```
```{r body_pages-1, include=T, eval=T, echo=T}
body_pages <-
  pages %>%
...
```
```{r body_rngs-1, include=F, eval=F, echo=F}
body_rngs <-
  body_pages %>%
...
```
```{r toc-1, include=T, eval=T, echo=T}
toc <-
  toc_pages %>%
  unnest_pages() %>%
  mutate_at(
...
```
```{r toc_ex-1, include=T, eval=T, echo=T}
toc_ex <-
  toc %>%
  filter(year == 2018) %>%
...
```
```{r toc_aug-1, include=T, eval=T, echo=T}
toc_aug <-
  toc %>%
...
```
```{r toc_aug_ex-1, include=T, eval=T, echo=T}
toc_aug_ex <-
  toc_aug %>%
  filter(year == 2018) %>%
...
```
```{r section_rngs-1, include=T, eval=T, echo=T}
section_rngs <-
  toc_aug %>%
...
```
```{r toc_n-1, include=T, eval=T, echo=T}
toc_n <-
  toc_sections %>%
...
```
```{r toc_n-2, include=F, eval=F, echo=F}
toc_n %>% pull(section_label) %>% levels()

```
```{r toc_content_n-1, include=F, eval=F, echo=F}
toc_content_n <- toc_n %>% subset_content()
toc_content_n

```
```{r toc_n_1yr-1, include=F, eval=F, echo=F}
toc_n_1yr <- toc_content_n %>%  filter(year == 2018)
toc_n_1yr

```
```{r toc_n_1yr_wfl-1, include=F, eval=F, echo=F}
toc_n_1yr_wfl <-
  toc_n_1yr %>%
...
```
```{r viz_toc_n_1yr-1, include=F, eval=F, echo=F}
viz_toc_n_1yr <-
  toc_n_1yr_wfl %>%
  ggplot() +
...
```
```{r viz_toc_n_1yr-2, include=F, eval=F, echo=F}
...
```
```{r toc_content_n1-1, include=F, eval=F, echo=F}
toc_content_n1 <- toc_content_n %>% filter(n == 1)
toc_content_n1

```
```{r viz_toc_content_n1-1, include=F, eval=F, echo=F}
viz_toc_content_n1 <-
  toc_content_n1 %>%
  group_by(year) %>%
...
```
```{r viz_toc_content_n1-2, include=F, eval=F, echo=F}
teproj::export_ext_png(
  viz_toc_content_n1,
  dir = .dir_viz,
  units = "in",
...
```
```{r section_rngs_n-1, include=F, eval=F, echo=F}
section_rngs_n <-
  toc_sections %>%
  subset_content() %>%
...
```
```{r section_rngs_n_1yr-1, include=F, eval=F, echo=F}
section_rngs_n_1yr <- section_rngs_n %>% filter(year == 2018)
section_rngs_n_1yr

```
```{r viz_section_rngs_n_1yr-1, include=F, eval=F, echo=F}
...
```
```{r viz_section_rngs_n_1yr-2, include=F, eval=F, echo=F}
teproj::export_ext_png(
  viz_section_rngs_n_1yr,
  dir = .dir_viz,
  units = "in",
...
```
```{r lines-1, include=T, eval=T, echo=T}
lines <-
  body_pages %>%
  mutate(
...
```
```{r lines-2, include=F, eval=F, echo=F}
# TODO:
...
```
```{r lines_aug-1, include=T, eval=T, echo=T}
lines_aug <-
  lines %>%
...
```
```{r rgx_zones-1, include=F, eval=F, echo=F}
...
```
```{r words-1, include=T, eval=T, echo=T}
words <-
  lines_aug %>%
  tidytext::unnest_tokens(
    output = word,
...
```
```{r words-2, include=F, eval=F, echo=F}
# words %>%
...
```
```{r words_aug-1, include=T, eval=T, echo=T}
.delim <- "QQ"
...
```
```{r words_filt-1, include=T, eval=T, echo=T}
words_filt <-
  words_aug %>%
  drop_words_generic() %>%
  anti_join(stop_words)
...
```
```{r lines_redux-1, include=T, eval=T, echo=T}
lines_redux <-
  words_aug %>%
...
```
```{r lines_redux-2, include=F, eval=F, echo=F}
lines_redux %>% count(line_type)

```
```{r lines_redux_filt-1, include=T, eval=T, echo=T}
lines_redux_filt <-
  words_filt %>%
  group_by(year, idx_page, idx_line, line_type) %>%
  summarise(line = paste(word, collapse = " ")) %>%
...
```
```{r words_pmi-1, include=F, eval=F, echo=F}
...
```
```{r ngrams-1, include=T, eval=T, echo=T}
ngrams <-
  bind_rows(
...
```
```{r n_k_max-1, include=T, eval=T, echo=T}
n_k_max <-
  # ngrams_filt %>%
  ngrams %>%
  count(k) %>%
...
```
```{r ngrams_filt_n-1, include=T, eval=T, echo=T}
ngrams_filt_n <-
  ngrams_filt %>%
  select(year, ngram, k) %>%
...
```
```{r ngrams_filt_n-2, include=F, eval=F, echo=F}
ngrams_filt_n %>% distinct(k, n_k_factor)

```
```{r ngrams_tfidf-1, include=T, eval=T, echo=T}
ngrams_tfidf <-
  ngrams_filt %>%
  count(year, ngram) %>%
...
```
```{r ngrams_tfidf_aug-1, include=T, eval=T, echo=T}
...
```
```{r ngrams_tfidf_aug-2, include=F, eval=F, echo=F}
set.seed(42)
...
```
```{r ngrams_tf_maxk-1, include=T, eval=T, echo=T}
...
```
```{r ngrams_tf_maxk_top-1, include=T, eval=T, echo=T}
ngrams_tf_maxk_top <-
  ngrams_tf_maxk %>%
  select(n, k, ngram) %>%
  filter(n == max(n))
...
```
```{r lines_tfidf-1, include=F, eval=F, echo=F}
# FIXME: Using this?
# lines_tfidf <-
#   lines_redux %>%
#   count(year, line) %>%
...
```
```{r lines_redux_n-1, include=F, eval=F, echo=F}
# FIXME: Using this?
# lines_redux_n <-
#   lines_redux %>%
...
```
```{r words_n-1, include=T, eval=T, echo=F}
words_n <-
  # words_aug %>%
  words_filt %>%
...
```
```{r words_frac-1, include=T, eval=T, echo=F}
# NOTE: "Manual" tfidf
...
```
```{r words_frac_filt-1, include=T, eval=T, echo=T}
words_frac_filt <-
  words_frac %>%
  # filter(n_word > 5) %>%
...
```
```{r words_tfidf-1, include=T, eval=T, echo=T}
words_tfidf <-
  words_n %>%
...
```
```{r words_tern-1, include=F, eval=T, echo=F}
words_tern <-
  words_tfidf %>%
  select(year, word, tf) %>%
  # group_by(year, word) %>%
...
```
```{r viz_words_tern-1, include=F, eval=T, echo=F}
...
```
```{r viz_words_tern-2, include=F, eval=T, echo=F}
teproj::export_ext_png(
  viz_words_tern,
  dir = .dir_viz,
  units = "in",
...
```
```{r viz_words_tern-3, include=F, eval=F, echo=F}
...
```
```{r viz_tfidf-1, include=F, eval=T, echo=F}
words_tfidf_filt <-
  words_tfidf %>%
  arrange(desc(tf_idf)) %>%
  mutate(
...
```
```{r viz_tfidf_chatter-1, include=F, eval=F, echo=F}
...
```
```{r viz_words_tfidf-2, include=F, eval=T, echo=F}
teproj::export_ext_png(
  viz_words_tfidf,
  dir = .dir_viz,
  units = "in",
