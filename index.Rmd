...
``` {r path_import, include=F, echo=F, eval=F}
...
``` {r setup-lazy, include=F, echo=F, eval=F}
if(file.exists(.path_import)) {
  load(.path_import)
}
```
...
``` {r procedure-1, include=F, echo=F, eval=T}
...
chr_words <- 'unique words'
chr_verb <- 'have more'
chr_verb_2 <- 'have less'
...
chr_hypothesis_2 <- 
  combine(chr_adj_topics, chr_verb_2, chr_words)
chr_hypothesis_into_2 <- 
  combine('"', chr_adj_topics, '"', chr_verb_2, chr_words)
```
...
is probably unsurprising `r emo::ji('stuck_out_tongue')`.) That is, they have
...
[^5]: A similar plot could have been made for the counts of `r chr_content`, but, as I
...
developed earlier---that `r chr_hypothesis`. The extension of this notion
can be stated explicitly as `r chr_hypothesis_into_2`.
